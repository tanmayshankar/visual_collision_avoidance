    ...
    if (numDims==8) 
      ...
      while (!exitCondition)        
        {   
            ...
            for (int action_index=0; action_index<numActions; action_index++)
                {   
                    for (int state_iter=0; state_iter<number_sample; state_iter++)
                        {   
                            for(int i=0; i<state_size; i++)
                            ...
                            err = getNeighbors(indicies, &gridQsa[action_index], neighX, neighY, numDims, discMat);
                            qVals[action_index] = interpN(numDims, currentState, neighX, neighY, EXTRAPMODE);

                            Q_val_bel[action_index] = qVals[action_index] * get_probability_belief(state_iter);
                        }
                }

            bestActionInd = 0; 
            // maxQsa = qV
            max_q_val = Q_val_bel[0];

            for(int i=1; i<numActions; i++)
                {   if (Q_val_bel[i]>max_q_val)
                        {   max_q_val = Q_val_bel[i];
                            bestActionInd = i; 
                        }
                }
